...
var as = ['laydate_box', 'laydate_nothis', 'laydate_click', 'LayDateSkin', 'skins/', '/laydate.css'];
...
Dates.viewYears = function(YY){
    ...
    Dates.each(new Array(168), function(i){
        if(i === 84) {
            str += '<li '+ (parseInt(as.year.value) === YY ? 'class="'+ as[2] +'"' : '') +' y="'+ YY +'">'+ YY +'年</li>';
        } else {
            str += '<li y="'+ (YY-84+i) +'">'+ (YY-84+i) +'年</li>';
        }
    });
    S('#laydate_ys').innerHTML = str;
	var h = ($("div.laydate_show").find("li.laydate_click").prevAll().length-4)*20;
	$("div.laydate_show")[0].scrollTop = h;
...
Dates.initDate = function(){
	...
	$("#laydate_Title").text(Dates.elem[as.elemv]);
    if(ymd.length < 3){
...
Dates.iswrite = function(){
    ...
    Dates.shde(as.oClearCon, !('iclearCon' in Dates.options ? Dates.options.iclearCon : 1));
};
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        div.innerHTML =  log.html = '<p id="laydate_Title">2222</p>'
        ...
        +'<div class="laydate_bottom">'
          ...
          +'<div class="laydate_btn">'
                ...
                +'<a id="laydate_clear" style="display: block;">清空</a>'
				+'<a id="laydate_Cancel" style="display: block;">取消</a>'
          +'</div>'
...
Dates.reshow = function(){
    Dates.each(Dates.query('#'+ as[0] +' .laydate_show'), function(i, elem){
		...
		$(elem).parent().removeClass("laydate_open");
    });
    return this;
};
...
Dates.creation = function(ymd, hide){
    ...
    if(hide){
		$(Dates.elem).trigger('focus');
		Dates.elem[as.elemv] = getDates;
		$(Dates.elem).trigger('change');
        Dates.close();
...
Dates.events = function(){
    ...
    Dates.each(S(log.box + ' .laydate_ym'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
			...
			$(elem).addClass("laydate_open");
            if(!i){
                log.YY = parseInt(as.year.value);
                Dates.viewYears(log.YY);
            }
			else{
				var h = ($("div.laydate_show").find("span.laydate_click").prevAll().length-3)*29;
				$("div.laydate_show")[0].scrollTop = h;
			}
        });
    });
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                Dates.stopmp(ev);
                // Dates.creation([this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0],true);
				// Dates.ymd[0] = this.getAttribute('y');
				// Dates.ymd[1] = this.getAttribute('m')-1;
				var x = document.getElementsByTagName("td");
				$.each(x,function(i,val){
					if(Dates.hasClass(val, as[2])){
						val.className = '';
					}
				})
				Dates.addClass(elem,'laydate_click');
				Dates.ymd[2] = this.getAttribute('d');
            }
		});
    });

	//取消
	as.oclear = S('#laydate_Cancel');
	Dates.on(as.oclear, 'click', function(){
		Dates.close();
	});
    //清空
    as.oClearCon = S('#laydate_clear');
    Dates.on(as.oClearCon, 'click', function(){
        $(Dates.saveElem.elem).text("").parent().next().hide();
        Dates.close();
    });
    //确认
    as.ok = S('#laydate_ok');
